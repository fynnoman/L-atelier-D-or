...
export default function JournalIndexClient() {
  const t = useT();
    ...
    <>
      <section className="relative pt-40 md:pt-52 pb-24">
        <div className="n-page">
          <PageEyebrow numeral={t.journal.eyebrowNum} label={t.journal.eyebrowLabel} className="mb-14" />
          ...
          <div className="grid grid-cols-12 gap-x-6 items-end">
            <div className="col-span-12 md:col-span-9">
              <LineReveal
                ...
                lines={[t.journal.title1, t.journal.title2, t.journal.title3]}
                delayStep={130}
      ...
      <section className="pb-32">
        <div className="n-page grid grid-cols-12 gap-x-6 gap-y-24">
          {CAHIERS.map((c, i) => {
            const flip = i % 2 === 1;
            const loc = localizeCahier(c, locale);
            return (
              <article
                key={c.slug}
              ...
              >
                ...
                >
                  <div className="flex flex-col gap-2">
                    <span className="n-mono opacity-60">{loc.date}</span>
                    <span className="n-mono opacity-60">{loc.read}</span>
                    <Link href={`/journal/${c.slug}`} className="n-link mt-4 self-start md:self-end">
                      {t.journal.readCahier}
                    </Link>
                  </div>
                </div>
              </article>
